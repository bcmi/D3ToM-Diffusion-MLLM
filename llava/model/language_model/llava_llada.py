...
class LlavaLladaForMaskedDiffusion(LLaDAModelLM,LlavaMetaForCausalLM):
    ...
    # csc_0704_逐层改forward函数，增加了visual_token_mask
    def forward(
        self,
        ...
        # csc_0702_加了return的visual mask        
        if inputs_embeds is None:
            (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds, labels,new_input_ids ,visual_token_mask) = self.prepare_inputs_labels_for_multimodal(input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities, image_sizes,return_inputs=True)
        #prompt_lengths = 
        #breakpoint()
        # hack starts here
        ...
        if dpo_forward:
            ...
            output =  super().forward(
                ...
                visual_token_mask=visual_token_mask, # 您在函数签名中用的是 visual_mask
    ...
    def generate(
        ...
        **kwargs,
        ...
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _, visual_token_mask) = self.prepare_inputs_labels_for_multimodal(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes)
        else:
            # breakpoint()
            
            inputs_embeds = self.get_model().embed_tokens(inputs)
            visual_token_mask = torch.zeros_like(inputs, dtype=torch.bool)
        # if DEBUG_PRINT_IMAGE_RES:
        #     print("Seq len:",inputs_embeds.shape[1])

        #return super().generate(position_ids=position_ids, attention_mask=attention_mask, inputs_embeds=inputs_embeds, **kwargs)
        return llada_generate(self.get_model(),inputs_embeds=inputs_embeds,position_ids=position_ids,attention_mask=attention_mask,visual_token_mask=visual_token_mask,merged=merged,**kwargs)
